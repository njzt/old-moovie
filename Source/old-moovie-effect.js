...
var OldMoovieEffect = new Class({
	...
	linesComponent: function() {

		var element = new Element('span', {
			styles: {
				display: 'none',
				position: 'absolute',
				top: this.areaPos.y + 'px',
				width: '1px',
				height: this.areaSize.y + 'px',
				background: '#ddd',
				zIndex: 99999
			}
		});
		return (new OldMoovieComponent(element, this.options.container, function() {
			element.setStyles({
				left: this.areaPos.x + Number.random(0, this.areaSize.x) + 'px',
				opacity: (Number.random(0, 3) / 10)
			})
		}.bind(this)));
	},

	scratchesComponent: function() {

		var element = new Element('img', {
			// get one of 3 gif images
			src: 'oldmovie-scratch-' + ((this.components.length % 3) + 1) + '.gif',
			styles: {
				display: 'none',
				position: 'absolute',
				zIndex: 99999
		...
		return (new OldMoovieComponent(element, this.options.container, function() {
			element.setStyles({
				left: this.areaPos.x + Number.random(0, this.areaSize.x) + 'px',
				top: this.areaPos.y + Number.random(0, this.areaSize.y) + 'px',
	...
	maskComponent: function() {
		...
		var element = new Element('img', {
			src: 'oldmovie-radial-mask.png',
			styles: {
				display: 'none',
				position: 'absolute',
				top: this.areaPos.y + 'px',
				left: this.areaPos.x + 'px',
				height: this.areaSize.y + 'px',
				width: this.areaSize.x + 'px',
				opacity: 0.1,
				zIndex: 99999
			}
		});
